...
- (void)saveButtonTap:(id)sender
{
	if (!self.companyNameTextfield.text.length) {
		...
		[self.loader show];
		__weak typeof(self) weakSelf = self;
		[[APIClient sharedInstance] createUserWithEmail:self.emailTextfield.text username:nil password:self.passwordTextfield.text languageId:@"" customerTypeId:BusinessAccount companyname:self.companyNameTextfield.text website:self.websiteTextfield.text phone:self.phoneTextfield.text avatar:nil withCompletion:^(id response, NSError *error, NSInteger statusCode) {
			if (error) {
